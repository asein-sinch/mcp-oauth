...
from .context_dump import dump_executor_context
...
class AdkAgentToA2AExecutor(agent_execution.AgentExecutor):
  ...
  async def _execute_inner(
    ...
    dump_executor_context(context, session)
    ...
    while attempt <= max_retries:
      ...
      try:
        async def _collect_response() -> str | None:
          ...
          async for event in runner.run_async(
            ...
            logger.info("[DEBUG ADK EVENT] type=%s, is_final=%s, content=%s", type(event).__name__, is_final, evt_content)
      ...
      if run_error:
        await updater.add_artifact(
            [types.Part(root=types.TextPart(text=f"⚠️ {run_error}"))],
            name="response",
        )
        await updater.complete()
        return
      ...
      if final_response_content is None:
        if attempt <= max_retries:
          ...
          logger.warning("[DEBUG] No text generated by LLM runner, producing fallback greeting.")
          await updater.add_artifact(
              [types.Part(root=types.TextPart(text="Hello! I am the Sinch Messaging Agent. How can I help you today?"))],
              name="response",
          )
          await updater.complete()
          return
